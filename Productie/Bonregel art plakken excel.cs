...
public class RidderScript : CommandScript
{
	public void Execute()
	{
		...
		foreach (var myString in clipboardData.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries))
		{
			...
			string ItemCode = myStrValues[1].ToString();
			...
			else
			{
				...
				rsJoborderItem.Fields["QUANTITY"].Value = Convert.ToDouble(myStrValues[0]);
				//rsJoborderItem.Fields["LENGTH"].Value = Convert.ToDouble(myStrValues[3]);
				//rsJoborderItem.Fields["WEIGHT"].Value = Convert.ToDouble(myStrValues[6]);
