...
public class RidderScript : CommandScript
{
	...
	public void Execute()
	{
		...
		{
			ScriptRecordset rsItemRow = this.GetRecordset("R_JOBORDERDETAILITEM", "", "PK_R_JOBORDERDETAILITEM = " + (int)record.GetPrimaryKeyValue(), "");
			rsItemRow.MoveFirst();
			...
			string afwerking = rsItemRow.Fields["AFWERKING"].Value.ToString();
			string tekening = rsItemRow.Fields["CAMGEOMETRY"].Value.ToString();
			string TAG = rsItemRow.Fields["CAMPARAMETER"].Value.ToString();
			string DIMW = rsItemRow.Fields["DIM_W"].Value.ToString();
			string Tekla = rsItemRow.Fields["TEKLA_FASE"].Value.ToString();
			
			//alleen nu
			decimal verfvlak = Convert.ToDecimal(rsItemRow.Fields["PAINTAREA"].Value.ToString());
			decimal gewicht = Convert.ToDecimal(rsItemRow.Fields["WEIGHT"].Value.ToString());
			...
			rsItemRow.Fields["AFWERKING"].Value = afwerking;
			rsItemRow.Fields["CAMGEOMETRY"].Value = tekening;
			rsItemRow.Fields["CAMPARAMETER"].Value = TAG;
			rsItemRow.Fields["DIM_W"].Value = DIMW;
			rsItemRow.Fields["TEKLA_FASE"].Value = Tekla;

			//alleen nu
			rsItemRow.Fields["PAINTAREA"].Value = verfvlak;
			rsItemRow.Fields["WEIGHT"].Value = gewicht;
